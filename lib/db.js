  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      if (now > resetDate) {
        ...
        await sql`
          UPDATE users 
          SET listings_used = 0, reset_date = ${nextMonth}
          WHERE id = ${user.id}
        `
  ...
  try {
    const result = await sql`
      ...
      VALUES (
        ...
        ${listingData.title},
        ${JSON.stringify(listingData.bullets)},
        ${listingData.description},
        ${JSON.stringify(listingData.metaData)},
        ${listingData.htmlOutput},
        ${listingData.seoScore},
        ${listingData.conversionScore},
        ${listingData.readabilityScore},
        ${listingData.errorScore}
      )
      RETURNING *
    `
